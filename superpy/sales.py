...
"""
        while True:
            try:
                ...
                table.add_column("VALUE ERROR", style="red")
                table.add_column("", style="dim")
                table.add_row("Not a valid value. Please enter a valid value", "Example: [1.99]")
                console.print(table)
    ...
    if available_products != None:
        if quantity <= len(available_products):
            ...
            table.add_column('PRODUCT', style='dim')
            table.add_column('CURRENT PRODUCT(S) IN STOCK')
            table.add_row(f"{name_product}", f"{len(available_products)}")
    ...
    if available_products != None:
        while True:
            try:
                amount_of_quantity = quantity 
                if amount_of_quantity <= len(available_products):
                    break
                else:
                    table = Table(title="UPDATE INVENTORY", show_header=True, header_style="steel_blue")
                    table.add_column('PRODUCT', style='dim')
